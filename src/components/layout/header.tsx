...
export function NavbarContainer(props: INavbarProps) {
	return (
		<header class="header sticky top-0 left-0 w-full py-2 sm:py-5 z-20 px-4">
...
function NavbarLogo(props: INavbarLogoProps) {
	return (
		<A href="/" class="relative h-8 w-8 sm:h-10 sm:w-10 navbar-logo">
			<img
...
export function NavbarDownloadButton(props: INavbarDownloadButtonProps) {
	return (
		<A
			download
			href={props.resourcePath}
			class="navbar-download-btn hidden group py-3 px-4 sm:pr-10 sm:pl-8 sm:py-6 bg-primary rounded-[100px] text-white font-light sm:flex items-center gap-4 lg:hover:bg-transparent lg:hover:ring-2 lg:hover:ring-primary lg:hover:ease-in-out lg:hover:duration-300 hover:transition-all"
		>
			<span class="lg:group-hover:text-primary text-sm sm:text-base">
				Download resume
			</span>
